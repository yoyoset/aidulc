...
pub enum ProgressEvent {
    StageStart { stage: String, ts: i64 },
    StageProgress { stage: String, current: i64, total: i64, ts: i64 },
    StageDone { stage: String, current: i64, total: i64, ts: i64 },
    SentenceDone { sentence_index: i64, status: String, ts: i64 },
    JobDone { exit_code: i64, message: String, ts: i64 },
    Error { message: String, detail: Option<String>, ts: i64 },
}

pub fn parse_progress_line(line: &str) -> Result<ProgressEvent, String> {
    let v: serde_json::Value = serde_json::from_str(line).map_err(|e| format!("NDJSON 解析失败: {e}"))?;
    ...
    match typ {
        ...
        "stage_progress" => Ok(ProgressEvent::StageProgress { stage, current, total, ts }),
        "stage_done" => Ok(ProgressEvent::StageDone { stage, current, total, ts }),
        "sentence_done" => Ok(ProgressEvent::SentenceDone {
            sentence_index: v.get("sentence_index").and_then(|s| s.as_i64()).unwrap_or(0),
...
mod tests {
    ...
    fn parses_sentence_done() {
        let e = parse_progress_line(r#"{"type":"sentence_done","sentence_index":5,"status":"ok","ts":1}"#).unwrap();
        assert!(matches!(e, ProgressEvent::SentenceDone { sentence_index: 5, status, .. } if status == "ok"));
    }

    #[test]
    fn parses_error_with_detail() {
        let e = parse_progress_line(r#"{"type":"error","message":"模型问题","detail":"GGUF 缺失","ts":1}"#).unwrap();
        match e {
