...
- **P2 删除学习档案后, 该档案的生词变成永久不可见的孤儿数据**:
  ...
  ⏸ 需要产品决策, 见下: 可选方向① 生词本页加"档案"筛选下拉(釜底抽薪——直接
  解决"非 default 档案的生词看不到"这个根问题, 工作量最大但最彻底)② 删档案前
  查一下这个档案下有没有生词, 有就警告"还有 N 个生词, 确定删除档案吗"(不阻止
  删除, 只是提醒, 工作量小)③ 删档案时给个选项"顺带删除这些生词"还是"改挂到
  default 档案下"(数据处置方式本身就是要问用户的, 不该由代码替用户决定)。
