...
- **跟读时间轴校准**: 高亮和声音对不上时, 顶栏 ⏱ → 「高亮提前 / 高亮延后」±0.5s ±2s,
  或复位本章。**整章一起平移**, 存库下次打开还在。三条实测教训写在
  `views/reader/sync_calibrator.js` 里: 面板不出现裸的 ±ms(数值方向与观感方向天生相反,
  没人能一次按对); 做过的「自动对齐」(点出自己正听到的那一句)已删除 —— 句子常有十几秒长,
  错位小于句长时"听到的"就是"高亮的"那一句, 按句选的分辨率天生够不着; 分段锚点
  (从第 N 句起生效)也已改成整章 —— 接缝处必然把一边的句子压成零长度, 结果退回上一句
  就没有声音。
