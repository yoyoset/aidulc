...
- **P2 词典发音与正文朗读是两套引擎, 音色不统一**: 整书朗读用备料阶段预生成的离线
  ...
  ⏸ 需要产品决策, 见下: 可选方向① 单词发音也走本地 TTS 引擎(音色统一, 但要为
  "单词"这种极短输入单独起一次引擎调用, 有延迟/资源开销, 且不是所有 TTS 引擎都
  擅长单字发音)② 保持现状(系统语音, 免费无延迟, 接受音色不统一)③ 只在已经跑
  过整书 TTS 的场景(音色已确定)才提供"用本书音色发音"选项, 没生成音频的书退化
  回系统语音——工作量最大但体验最一致。
