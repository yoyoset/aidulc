...
## ⚠️ 银河系章节错乱: 修复前处理的产物, 别当基准 (2026-08-08 补充)

Hitchhiker's Guide 的 38 章 / 单章 5351 句 / 21972 句 这套数字, 是 **R3.1 修 EPUB2
`toc.ncx` 支持之前**用 spine 每文件一章 + `[[HEADING]]` 被当垃圾过滤掉的旧逻辑跑出来的,
所以才出现 1 句的碎片章和 5351 句的巨章。**存量 3 本按用户决定不重跑**——所以这三本书的
现有书包仍是错乱分章, 不是"正确基准"。将来若要拿这本书做回归, 必须用 R3.1 之后的 loader
重新处理一遍再对比, 否则拿错乱当基准会把回归验证带偏。

同期修好的还有 (R3.1-R3.3, 2026-08-08):
- EPUB2 的 `toc.ncx` 目录解析 (此前只认 EPUB3 的 nav.xhtml, 认不出来退化成 spine 逐文件)
- 大文件按 `[[HEADING]]` 二次切分 (此前 heading 标记被过滤掉)
- PyMuPDF 兜底 pdf/mobi/azw3/fb2 (可选依赖, 组件健康页可一键安装)
- `--preview-book` 处理前体检: 格式/目录来源/每章句数分布/异常信号 (碎片章/巨章)
