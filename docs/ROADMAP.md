...
- **P2 封面尺寸不可调**: `reader/styles/library.css` 的 `.book-cover` 写死
  ...
  ⏸ 需要产品决策, 见下: 可选方向① 简单二档切换(紧凑/宽松, 复用 K2-3 计划里的
  网格/列表)② 连续尺寸滑杆(更灵活但要多做一套响应式断点)③ 暂不做, 并入 K2-3
  一起规划(现状: K2-3 的网格/列表切换本身也还没做, 单独先做尺寸调节可能是白做)。
...
- **P2 备份导出边界不一致**: `.aidu-data` 备份(`transfer_service.rs`)只导出/导入
  ...
  范围不一样。
  ⏸ 需要产品决策, 见下(本轮复核后从"直接修"改判成需要问——原因是这次复核翻到
  `transfer_service.rs:7` 模块头注释明确写着"aidulc 导出: 只写 vocab/dictionaries,
  **绝不导出**...阅读状态", 这是一句显式的、看起来经过考虑的设计声明, 不是遗漏;
  只是 M7 R31 后来加 highlights 导出时没回头改这句注释, 造成"文档说绝不, 代码却
  在做"的不一致, 但不代表"绝不导出阅读状态"这个决定本身作废)。可选方向:
  ① 把 bookmarks/reading_state 也纳入导出, 同时更新那句过时注释(风险: 阅读进度
  按章节下标记录, 如果换设备后书被重新处理过、章节切分变了, 恢复的书签/进度可能
  指向错的章节——highlights 有同样风险但已经在用, 说明这个风险目前被接受)
  ② 维持现状不导出, 改注释更精确说清"摘录导出、阅读进度不导出"分别的理由,
  不是笼统一句"绝不"③ 只导出 bookmarks(书签是用户主动动作, 丢了更可惜), 阅读
  进度(reading_state, 相对没那么珍贵、且下次打开会自然重新累积)继续不导出。
...
- **P2 无跨书搜索/标签分类**: 摘录面板只加载当前书(`highlights.js:29-34`), 后端
  ...
  ⏸ 需要产品决策, 见下(不是简单加个搜索框——摘录面板现在是阅读器内的浮层,
  跨书搜索命中另一本书的摘录后"怎么跳过去"涉及切书/换阅读器实例, 是导航流程
  设计, 不只是查询能力): 可选方向① 摘录面板本身只加"当前书内搜索"(小步、
  低风险, 用 `list_by_book` 已有数据前端过滤即可), 跨书搜索单独开一个"我的
  摘录"独立页面(类似生词本页), 列表按书分组, 点击进入对应书的对应章节
  (工作量中等, 但要处理"目标书当前没打开"这种跨路由跳转)② 标签/分类
  在 `Highlight` 加 `tags: Vec<String>` 字段, 摘录时可选打标签, 独立页面支持
  按标签筛选(在①基础上叠加, 数据层面要写迁移)③ 暂不做独立页面, 先只做
  当前书内搜索/按章筛选(先解决面板本身连筛选都没有的问题, 跨书搜索留到
  真的有人反馈"想不起来在哪本书记过"再做——现在是审计推导出的需求, 不是
  实测反馈)。
...
- **P3 摘录与生词本没有关联**: `Highlight` 无 lemma 引用字段, 摘录的 `.hl-span`
  ...
  ⏸ 需要产品决策, 见下: 可选方向① 只做展示层关联(不改 `Highlight` 表结构, 渲染
  摘录时实时比对句内 token 是否命中当前生词表, 命中的词加个提示样式——工作量小,
  纯 UI, 数据关系仍是隐式推导)② `Highlight` 加 `lemma_refs` 字段, 保存摘录时
  记录句内命中的生词 lemma 列表(数据层面显式关联, 能支持"看这条摘录学过哪些词"
  这类查询, 但要写一条迁移+改 upsert/list 接口)③ 暂不做, 这条本身优先级低
  (概率:摘录句子恰好包含生词, 后果:少一层联想, 不影响核心功能)。
...
- **P2(跨域汇总, 与"阅读器"域发现叠加)同步范围与备份范围完全一致地缺同一批表**:
  ...
  ⏸ 需要产品决策, 见下(与上面「阅读器」域 K1 附近"备份导出边界不一致"是同一批
  数据的两个不同机制, 建议一起定): 可选方向① 同步范围也扩大到 bookmarks/
  reading_state(实时同步比手动备份多一层考虑——CF Worker 免费额度写配额
  1000/天, 阅读进度比生词变化频繁得多, 扩大范围可能明显吃配额, 需要评估)
  ② 维持"只同步学习资产, 阅读进度设备本地"的现状, 但在设置页/同步状态说明里
  显式写清楚这个边界(现在完全没有任何文案提到这一点)③ 折中: 只把 bookmarks
  (用户主动动作)纳入同步, reading_state(自动记录的进度)继续设备本地。
...
- **P3 模型下载与备料任务无资源互斥, 只防重复下载同一文件**:
  ...
  要在下载入口感知"当前 running 任务绑定了哪些模型 id")。
...
- **P3 无设置搜索, 无单项"恢复默认值"**: 5 个 tab 里塞了相当多控件(书库位置/
  ...
  ⏸ 需要产品决策, 见下(两个子问题独立, 优先级/工作量都不同): 可选方向①
  只做"单项恢复默认值"(每个控件旁加一个小的"↺ 默认"按钮, 工作量小, 不需要
  搜索这种发现型 UI, 5 个 tab 控件数量其实不算多, 用户知道自己要改哪个)②
  只做设置搜索框(输入关键词高亮/跳转到对应 tab 对应控件, 工作量中等, 要给
  每个控件配可搜索的关键词元数据)③ 两个都做(工作量最大)④ 都不做, 现有的
  "重新运行首次向导"已经能覆盖"设置乱了想重来"这个最典型场景, 单项级颗粒度
  是不是真需要存疑(5 个 tab、控件数量有限, 用户自己找到并改回去成本不算高)。
- **P3 无纯配置导入/导出(区别于数据备份)**: 现有备份(`vocab_view.js:35-49`
  ...
  ⏸ 需要产品决策, 见下(不是纯粹的技术活——"配置"这个词的边界要先定清楚,
  才知道该导出什么): 可选方向① 只导出阅读器设置(`reader_settings` 表, 字体/
  主题/播放速度这类纯个人偏好, 风险最低——不含任何服务端点/路径, 换设备后
  这些本来就该带走)② 连同步后端列表/在线引擎 endpoint 一起导出(信息量更完整,
  但要明确排除 token/密钥, 且这些字段本身可能包含用户自建服务的内网地址,
  算不算"敏感"需要用户自己判断)③ 暂不做, 优先级判断依据: 这条本身是从
  "换设备体验不连续"推导出来的，没有实测反馈说明有多少用户真的换设备,
  比①②两个方向的工作量分歧更值得先问用户"到底需不需要"。
