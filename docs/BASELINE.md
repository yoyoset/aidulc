...
2. **本节数字仍未核实**——bug 修复只保证以后的运行路径可预测，不能倒推 Wolf 21/Breath/
   Hitchhikers 这三次历史运行当时落在了哪。需要实际启动应用、看新加的启动日志能不能找到，
   或者用户提供当时的实际路径；找不到就用真实的新数据重新处理这三本书验证端到端可用性。
3. 若确认数据已丢失，如实标注"数据已丢失，此表格永久保留自述版本作为历史记录"，
   不要删除这段追溯。
4. 补充产品需求（任务 #7）：书库位置设置界面可见可改 + 书包导出导入，从根源上避免
   "不知道数据在哪"再次发生——目前只修了"路径解析不出 bug"，用户仍然只能看日志才知道
   书库在哪，还没有真正的界面入口。
