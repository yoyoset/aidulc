...
- **跟读时间轴校准**: 高亮和声音对不上时, 顶栏 ⏱ → 「高亮提前 / 高亮延后」±0.5s ±2s 微调,
  或复位本章。**调整从打开面板那一句起生效, 前面的不受影响**, 存库下次打开还在。
  两处实测教训写在 `views/reader/sync_calibrator.js` 里: 面板不出现裸的 ±ms(数值方向与
  观感方向天生相反, 没人能一次按对); 一度做过的「自动对齐」(点出自己正听到的那一句)
  已删除 —— 句子常有十几秒长, 错位小于句长时"听到的"就是"高亮的"那一句, 差值恒为 0,
  按句选的分辨率天生够不着。
