...
- **P3 模型下载与备料任务无资源互斥, 只防重复下载同一文件**:
  ...
  ⏸ 需要产品决策, 见下(值不值得为一个低概率边界情况投入工程量是判断题, 不是我
  该替用户拍板的): 可选方向① 不处理——`preflight_check` 在任务入队前已经查过
  模型文件存在, 真撞上"下载中途切 TTS 阶段"这个时序算是用户主动做了两件事的
  巧合, 撞上时任务会在 TTS 阶段报"文件不存在/不完整", K17 刚做的「去修模型」
  跳转+下载单能兜底恢复, 不是死路② 备料任务开始前锁定当时绑定的模型文件(比如
  记录 mtime/大小快照), TTS 阶段前二次校验, 校验失败给明确的"模型文件在处理
  过程中被修改"提示而不是笼统报错(比①多一点点用户体验, 工程量也小)③ 备料
  任务运行期间禁止对它依赖的那个模型发起下载(真正的资源互斥, 工程量最大——
  要在下载入口感知"当前 running 任务绑定了哪些模型 id")。**此条(下载 vs 备料
  任务本身的竞态)维持 ⏸, 未拍板。**
