...
- **P3 📝 已决策(2026-08-14, 用户拍板)封面尺寸不可调**: `reader/styles/library.css` 的 `.book-cover` 写死
  ...
  **决策: 不单独做, 并入 K2-3 网格/列表视图切换一起规划**(K2-3 本身的网格/列表
  切换还没做, 单独先做尺寸调节会是白做——等 K2-3 排期时一起设计)。
- **P2 ✅ 已修复(2026-08-14)老书没有回填路径**: K2-2 封面管线只在新跑一次完整处理(到 pack 阶段)时生效。
...
- **P3 📝 已接受现状(2026-08-14 复核)无封面手动上传**: 源书本身没有封面(纯文本 TXT、无封面的 EPUB)时, 占位块
  是唯一选项, 没有"用户自己传一张图当封面"的入口。低优先级(离线优先定位下,
  这类锦上添花的个性化不紧急)。复核结论: 维持现状, 不是需要用户拍板的决策题
  (方向单一——要做就是"加个文件选择器+拷贝到 pack_dir+更新 cover 字段", 没有
  设计分歧), 只是优先级排在后面, 需要时再做, 先记录不排期。
- **P3 📝 已接受现状(2026-08-14 复核)封面与"原书 vs 译本"关系未理清**: 一本原书可以生成多个不同参数的译本
...
- **P2 ✅ 已修复(2026-08-14, K29, 用户拍板: 建常驻推理服务完整方案)词典发音与正文朗读是两套引擎, 音色不统一**: 整书朗读用备料阶段预生成的离线
  ...
  **产品决策(用户拍板)**: 阅读时正文查词继续用系统机械音(用户确认"机械没有问题",
  不改); 生词本/复习页加独立的本地 TTS 发音(跟正文朗读同一引擎), 且进入阅读器
  时预热, 减少首次点发音的等待。**修复**:
  - Python 侧新增 `application/tts_server.py`(常驻语音合成守护), 协议对称抄
    `dict_server.py` 已验证过的模式(stdin/stdout 逐行 JSON); `cli.py` 加
    `--tts-server`/`--tts-model`/`--tts-language`。`KokoroEngine.synth()` 早就是
    单句合成接口, `get_engine()` 本身在进程内做单例缓存, 不用改引擎本身。
  - Rust 侧新增 `infrastructure/tts_daemon.rs`, 完整照抄 `dict_daemon.rs` 已经
    生产验证过的常驻守护模式(懒启动/120s 空闲回收/超时安全读+kill重置/stderr
    尾部上屏), 不是重新设计一套。备料任务启动前(`jobs::spawn::spawn_prep`)和
    应用退出时都调 `tts_daemon::stop()`, 跟词典守护同一组调用点——备料任务(尤其
    TTS 阶段)要独占显存, 两个常驻守护不能同时占着。
  - 新命令 `tts_synth_word`(合成一个词, 返回 base64 WAV)、`tts_prewarm`(进入
    阅读器时调, 真实合成一个短词触发模型加载, 不等待/不阻塞阅读, 未配置语音
    模型时静默跳过)。
  - 生词本页(`vocab_view.js`)行菜单加"🔊 发音"; 复习页(`review_view.js`)原有
    的三级降级(①有来源句音频→播放书里的原声②否则→ speechSynthesis)插入新的
    中间层, 变成①原声②本地 TTS 常驻守护③speechSynthesis(纯降级, 无 Tauri 环境
    时才落到这里)。
  - 6 个新 Python 测试(`test_tts_server.py`, 协议逐行/失败不崩/CLI 契约, 照抄
    `test_dict_server.py` 的写法)+ 4 个新 Rust 测试(`tts_daemon.rs`, 挂死超时/
    秒退上屏/幂等, 照抄 `dict_daemon.rs` 的写法)。
...
- **P2 ✅ 已修复(2026-08-14, K27, 用户拍板: 可勾选, 各是独立边界, 可全选)备份导出边界不一致**: `.aidu-data` 备份(`transfer_service.rs`)只导出/导入
  ...
  范围不一样。**修复**: `export_aidu_data` 补上 `bookmarks`/`reading_state` 两类
  (按 `book_key` 分组, 与既有 `highlights` 同一惯例); `import_aidu_data` 对称补上
  导入路径(没有 `updated_at` 暴露给应用层, 做不了 vocab 那种"谁新谁赢"合并, 按
  "备份里有就覆盖本地"这种最朴素的备份恢复语义)。新增 `export_aidu_data_scoped`
  按用户勾选的类别过滤(5 类: 生词/词典/摘录/书签/阅读进度, 各自独立勾选, 可全选);
  内部自动安全备份(H5 剔词/删除学习档案等场景)继续走不带 scope 的
  `export_aidu_data`, 永远全量, 不受这个用户手动导出入口的勾选影响。生词本页
  「备份」按钮改成先弹类别勾选框, 默认全选。2 个新 Rust 测试(书签+阅读进度往返/
  scope 过滤)。
...
- **P2 ✅ 已修复(2026-08-14)无到期复习的全局提醒**: 到期数字只在生词本页内的"今日队列"卡片显示
...
- **P1 ✅ 已修复(2026-08-14, commit 4c2656b)多用户切人后摘录面板可能静默空白**:
  ...
  **F29(命令参数门禁)catch 不住这个问题**: F29 校验的是 `bridge.js` 里那一条
  ...
  功能实质不可用)。
- **P2 ✅ 已修复(2026-08-14, K26, 用户拍板: 跨书搜索不需要, 改做带日期时间的书签)无跨书搜索/标签分类**: 摘录面板只加载当前书(`highlights.js:29-34`), 后端
  ...
  **决策**: 跨书搜索/标签都不做——用户实际诉求是"书内知道什么时候读到哪里",
  不是跨书查摘录。**修复**(K26): `reading_state.bookmarks` 每章数组的元素从纯
  句下标升级成 `{i, at}`(创建时间 ms), 迁移 v29 处理存量数据(老行没有单条时间戳,
  用该行 `updated_at` 兜底); `BookmarkPanel`(`bookmarks.js`)从 `Set<下标>` 改成
  `Map<下标, 创建时间>`, 面板按时间倒序列书签, 每条带"今天几点/哪天几点"的时间
  展示。3 个新 Rust 测试(纯数字升级/已是新格式幂等跳过/历史迁移链联动)。
...
- **P3 📝 已决策(2026-08-14, 用户拍板: 不需要)摘录与生词本没有关联**: `Highlight` 无 lemma 引用字段, 摘录的 `.hl-span`
  渲染标记和生词的 `.saved` 标记是 `atomic_block.js` 里互不相干的两条通道——摘录
  一句包含生词的句子后, 看不出"这句里有我正在学的词"。**决策: 不做**——用户
  确认摘录就是"金句摘录", 是独立的功能, 不需要跟生词本关联。
...
- **P2 ✅ 已修复(2026-08-14)UI 文案与实际行为不符**: `reader/views/settings/sync_tab.js:146` 的
  ...
  分别 grep), 是确认级别的发现。
- **P2 ✅ 已修复(2026-08-14, K22, 用户拍板: 维持现状+补文案)同步范围与备份范围完全一致地缺同一批表**:
  ...
  设备本地状态"这个未被明说的设计决策。
  **决策: 维持同步范围只覆盖生词表**(用户确认"同步只是生词数据, 不含其他"就是
  想要的边界, 不是遗漏)。**修复**: `sync_tab.js` 加一行显式说明"同步范围: 只同步
  生词/背单词进度, 不含摘录、书签、阅读进度和阅读器设置", 之前只有 h2 标题隐含
  这个意思, 现在边界看得见。备份导出范围(区别于本条的实时同步)另见 K23。
- **P3 📝 结论: 接受现状不改(2026-08-14)手机端 token 存储强度弱于桌面端**: 桌面走 Windows Credential Manager
  (`services/credentials.rs:8-11`, 不落明文), 手机端 token 明文存 IndexedDB
  (`cloud/mobile/adapter.js:101-115`)。移动 Web 场景没有系统级密钥库可用是常见
  限制, 不算错误, 但同一套账号体系两种保密强度值得记录, 优先级低。复核后维持
  原判断: 这不是缺陷, 是浏览器沙盒的固有约束(没有可替代方案), 不需要用户拍板
  也不需要改代码, 记录在案即可。
...
- **P2 ✅ 已修复(2026-08-14, K17)完整性检测到文件丢失/损坏后没有直接修复入口**:
  ...
  **修复**: `router.js` 支持 hash 带 query(`#/models?focus=tts`, 路由匹配只看
  `?` 前的部分); `library_view.js` 的 preflight 失败面板、`prep_view.js` 的
  「去修模型」按钮(此前误跳到只有版本检查、没有下载能力的"设置→依赖组件"页,
...
- **P2 ✅ 已修复(2026-08-14)"取消"语义不完整, 声明的状态值和已注册命令没有真正被用上**:
  ...
  的中间语义。
- **P2 ✅ 已修复(2026-08-14, K20)长任务无 ETA, 只有百分比**: `prep_view.js:325-331` 每行进度条只显示
  `job.progress`(7 阶段权重均分算出的百分比), 没有任何时间预估。翻译/讲解一本
  长篇小说可能要跑数小时, 用户只能看着百分比猜, 不知道是"还要 5 分钟"还是
  "还要 5 小时"——这类批处理任务本该是 ETA 最有价值的场景, 现在完全没有。
  **修复**: `_formatEta(job, pct)` 用已耗时(`Date.now() - job.created_at`)/已完成度
  线性外推剩余时间, 拼进百分比文本("57% · 约剩 12 分钟")。不追求精确预测(各
  阶段耗时本来就不均匀, TTS 通常比 parse 慢很多), 只给量级参考; 进度 <5%(样本
  太少)或 ≥95%(收尾阶段常变慢, 线性外推容易报错)时不显示, 避免给出误导性数字。
  `prep_view.js` 601→648 行, 按 K8/K15 先例更新 `file_size_baseline.json`。
- **P3 ✅ 已修复(2026-08-14, K16)并发数硬编码为 1, 无用户可调项**: `job_orchestrator.rs:757-759` 的
  ...
  用户体验上等同于一个没解释的限制。
  **修复**: 不改并发策略本身(仍是刻意的显存安全设计, 不是产品决策空白点),
  只在"排队中"标题旁加 ⓘ 提示 + tooltip 说明原因(`prep_view.js` "排队中"标题、
  `prep.css::.prep-queue-hint`)。
- **P3 ✅ 已修复(2026-08-14, K30, 用户拍板"判断一下然后提示")模型下载与备料任务无资源互斥, 只防重复下载同一文件**:
  ...
  这是一个"理论上可能撞上但概率低"的边界情况。**决策**: 选原方向②/③之间的
  轻量折中——不做真正互斥锁(方向③工程量最大), 也不是完全不处理(方向①),
  照 K28"检测+提示"的同一模式: 点下载前查一下有没有 running 备料任务, 有就弹
  确认框说明风险("如果这个模型正好是它接下来要用的, 任务切到那个阶段时文件
  可能还没下完"), 用户自己选"仍然下载"还是"先不下"。**修复**: `models_view.js`
  新增 `_maybeWarnJobRunning()`(与 `reader_view.js::_maybeWarnBackgroundTasks`
  同一模式, 复用现成的 `job_list`+`AiduModal.confirm`), 接进两个下载入口
  (目录下载 `_downloadModel`、自定义模型下载表单)。查不到任务列表/没有
  running 任务时直接放行, 不阻断正常下载——这是体验提示, 不是安全校验。
- **P3 ✅ 已修复(2026-08-15, K31, 用户拍板"我没什么意见, 听你的")书库页排序/卡片大小固定, 用户不可调**:
  `library_view.js::_renderBooks()` 排序硬编码"最近打开优先(`last_opened_at` 降序)+
  其余按标题字母序"(F30), 卡片布局固定网格尺寸, 用户实测后指出"我的书页面, 展示
  方式、大小、排序, 这些都没有"。**修复**: 工具条加排序下拉(最近打开/书名A-Z/
  阅读进度/添加时间, `this._sortMode` 纯内存不持久化, 默认行为与历史完全一致);
  加卡片大小二态切换(紧凑/大图, `book-list--compact` class, `library.css` 补对应
  尺寸规则, 复用 `tokens.css` 阶梯令牌无裸值)。**网格/列表视图切换评估后跳过**:
  需要重写卡片内部 DOM 为横向布局, 改动面明显超过前两项之和, 属于硬凑, 留待
  单独排期(与上面 K2-3/封面尺寸那条历史待办是同一件事, 现在只是把排序/尺寸这
  两块先落地, 视图切换本身仍未做)。用 progate 实现, 编排方复核 git diff 通过。
- **P2 ✅ 已修复(2026-08-15, K32, 用户拍板"生词本会慢慢积累, TTS 也该是增量的")生词发音是现场合成不是预生成缓存**:
  K29 做的是"点击才现场调本地 TTS 合成, 不落盘, 播完即丢"。用户实测后指出这跟
  他设想的模型不一致——生词加入生词本时应该后台异步预生成音频存起来, 之后
  生词本/复习页播放的都是这份预生成音频, 不是每次现场合成; 查词面板(还没收进
  生词本前)继续系统机械音不受影响。**修复**: `commands/dictionary.rs` 新增
  `tts_cache_word`(幂等预生成, 未配置语音模型静默跳过)和 `vocab_read_cached_audio`
  (读缓存 base64, 未命中返回 null); `dictionary_panel.js` 加词成功后 fire-and-forget
  触发预生成, 不阻塞加词交互; `vocab_view.js::_playPronunciation` /
  `review_view.js::_speakWord` 改成缓存优先, 未命中才降级现有 `ttsSynthWord`
  现场合成(K29 路径保留作兜底); 生词本页头加"补全发音"按钮补齐存量生词。
  **设计上比拍板时最初设想更保守**: 没有新增 `vocab` 表列/没跑数据库迁移,
  改用"缓存文件是否存在"本身当作"是否已生成"的标记(缓存天然幂等, 文件在就是
  已生成)——这是编排方派活前重新评估后的简化, 本轮不能跑 build/test 验证迁移
  正确性, 不碰表结构风险更低, 功能效果等价。用 progate 实现, 编排方复核通过。
- **P2 ✅ 已修复(2026-08-14, K28, 用户拍板)阅读与后台备料任务无资源互斥提示**:
  上面那条是"下载 vs 备料任务"的竞态; 这条是用户提出的相关但不同的诉求——
  打开阅读器时如果后台正跑着备料任务(占显存/算力), 会拖慢查词等交互, 但此前
  完全没有提示, 用户不知道"现在查词慢是因为后台在跑东西"。**修复**:
  `reader_view.js::open()` 打开书之前先查一次 `job_list`, 如果有 `status='running'`
  的任务, 弹确认框"后台正在处理书籍, 要不要先暂停后台再阅读", 选"暂停后台"
  调用已有的 `pauseAll` 命令(不新建任务调度逻辑, 复用现成的暂停能力), 选"仍然
  阅读"直接放行——不阻断阅读路径, 纯粹是把"后台任务会影响交互速度"这个隐含
  事实变成用户可见、可选择的提示。顺带给 `AiduModal.confirm` 加了通用的
  `onCancel` 回调(之前取消只关窗口, 调用方拿不到"用户选了取消"这个信号)。

### 设置 (2026-08-14, settings_view.js + 5 个 settings/*.js tab + settings_repo.rs + services/config.rs)

现状枚举委派 flash 做的, 判断以下 4 条基于枚举结果, 逐条 grep 复核过。

- **P2 ✅ 已修复(2026-08-14, K21)删除学习档案后, 该档案的生词变成永久不可见的孤儿数据**:
  ...
  **产品决策(用户拍板, 2026-08-14): 生词本应该是独立的个人词典, 按 user 累加,
  不该跟阅读档案(profile)绑定——档案只决定"这本书用什么风格讲解", 跟"这个词
  该不该在我的生词本里"是两件事。** 根因其实是写入侧的错配, 不是读取侧缺筛选:
  `dictionary_panel.js` 加词时把"这本书挂的讲解档案"(如 kid)当成了生词的分区键,
  而 `vocab_view.js`/`review_view.js` 一直固定只读 'default'。**修复**: 引入
  `domain/vocab.rs::VOCAB_PROFILE_ID`("default"), `dictionary_service.rs::add_to_vocab`
  的 `VocabRepo` 调用(旧 context 读取、写入)一律用这个常量, 不再用调用方传入的
  阅读档案 profile_id(`dictionary` 表的释义缓存仍按 profile_id 分——不同档案讲解
  深浅不同, 这部分继续隔离是对的); `lookup_local` 的 `in_vocab` 判断同理改用固定
  常量, 否则同一个词换个档案读书查词会显示"未加入生词本"。**v28 迁移**归并存量的
  非 default 生词行: 同一个 (user, lemma) 如果 default 下已有一行, 保留
  `updated_at` 更晚的那份(SRS 进度更可信); 没有则直接把行搬过去, 数据不丢。
  3 个新 Rust 测试锁定行为(归并成 1 行 / 无冲突词直接搬迁 / 不再有任何非-default
  行)。`store_mod.rs` 1518→1674 行, 按 K8 先例更新基线。此前"档案筛选下拉/删档案
  警告"两个候选方案已不适用——问题从根上不存在了。
- **P2 部分已修复(2026-08-14)应用本体无版本号展示 / 无「关于」页面 / 无更新检查**
  (版本号展示已加, 「关于」整页/在线更新检查未做, 见下): 全仓 grep「关于」
  ...
  ⏸ 剩余部分(「关于」整页 + 在线更新检查)需要产品决策, 见下: 可选方向①
  只做静态「关于」页(许可证/依赖列表/项目主页链接), 不联网, 符合离线优先定位,
  工作量小②「关于」+ GitHub Release 在线更新检查(`downloader/mod.rs` 已有
  `github_release_asset_url` 可复用的下载基础设施), 但要回答"离线优先的软件
  要不要默认联网探测更新"这个定位问题, 至少要做成默认关闭/用户主动点按③
  暂不做整页, 现有"设置里一行版本号"已经解决"报 bug 说不清版本"这个最痛的点,
  「关于」页本身优先级不高, 先留白。
- **P3 ✅ 已修复(2026-08-14, K25, 用户拍板: 只做单项reset)无设置搜索, 无单项"恢复默认值"**: 5 个 tab 里塞了相当多控件(书库位置/
  ...
  **修复**: 「阅读显示」tab(`reading_tab.js`)的 主题/主题色/儿童模式 三个控件
  旁各加"↺ 默认"按钮, 默认值取自 `settings_repo.rs::Default`(与后端 schema
  默认值同一份真相: theme='light'/palette='clay'/child_mode=false)。范围只做
  这一个 tab——其余 4 个 tab(书库位置/在线引擎/档案/同步后端/依赖组件)的
  "默认值"本身没有 schema 层面的单一真相(书库路径、同步后端列表这类没有
  "默认值"这个概念, 强行加 reset 按钮语义上说不通), 留给以后按需扩展。
  搜索框(可选方向②)不做, 匹配用户"只做reset"的拍板。
- **P3 📝 已决策(2026-08-14, 用户拍板: 不需要)无纯配置导入/导出(区别于数据备份)**: 现有备份(`vocab_view.js:35-49`
  ...
  字体/主题、在线引擎配置这些要重新设一遍, 体验不连续。**决策: 不做**——用户
  确认换设备场景不是当前的痛点, 不为这个投入工作量。
