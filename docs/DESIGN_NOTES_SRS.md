...
### 第 7 处偏离: word_key = 词头小写, 无词性 (P2, 2026-08-10 补记)

- **设计稿 §03**: word_key = 词头小写 + 词性(与书无关)。
- **实现**: `cloud/worker/src/index.js:71` 的 `wordKey()` 只有词头小写; 本地
  `vocab` 主键 `user:profile:lemma` 也是 lemma 无词性。
- **判定**: 这是**有意的简化**, 不改。加词性要动本地主键 + 服务端 KV 键 + 两端序列化,
